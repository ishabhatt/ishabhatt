...
- **UI automation** with Playwright, Selenium, Cypress, Robot Framework
- **API and backend testing** with Postman, Rest Assured, gRPC, GraphQL, REST
...
- Increased automation coverage from **~10% to ~90%**
- Reduced incident detection time by **~30%**
- Improved test efficiency by **~40%**
- Reduced manual regression effort by **~50%**
- Improved automation reliability through reusable libraries, better test data strategy, and stronger failure triage
...
### Test Automation
`Playwright` `Selenium WebDriver` `Robot Framework` `Cypress` `PyTest` `TestNG`

### API / Backend
`Postman` `Rest Assured` `REST` `gRPC` `GraphQL` `Swagger/OpenAPI` `Contract Testing`

### Languages
`Java` `Python` `JavaScript` `TypeScript` `Go` `Shell Scripting`

### CI/CD / DevOps / Cloud
`GitHub Actions` `GitLab CI` `Jenkins` `Azure Pipelines` `Docker` `Kubernetes` `AWS` `Azure` `GCP`

### Performance / Observability
`k6` `JMeter` `Prometheus` `Grafana`

### Collaboration / Productivity
`Jira` `Confluence` `Zephyr` `TestLink` `Azure DevOps` `ChatGPT` `Claude` `Aider`
